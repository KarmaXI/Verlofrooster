    ...
    return items.map(item => {
        ...
        dateFields.forEach(field => {
            if (processedItem[field]) {
                ...
                // Check if date is valid (not NaN) and not a default/epoch date (1970)
                if (!isNaN(dateObj.getTime()) && dateObj.getFullYear() > 1970) {
                    // If valid, ensure consistent ISO format
                    processedItem[field] = dateObj.toISOString();
                } else {
                    console.warn(`[Machtigingen] Invalid date found in field ${field}:`, processedItem[field]);
                    // For invalid dates, set to null to avoid comparison issues
...
async function getLijstItemsAlgemeen(lijstConfigKey, selectQuery = "", filterQuery = "", expandQuery = "", orderbyQuery = "") {
    ...
    const lijstConfig = typeof window.getLijstConfig === 'function' ? window.getLijstConfig(lijstConfigKey) : null; // Gebruik window.getLijstConfig
