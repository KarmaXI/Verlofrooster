...
function setupModalEventHandlers() {
    ...
    saveBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        for (const indicator of window.dagenIndicators.indicators) {
          const success = await window.dagenIndicators.saveToSharePoint(
            indicator
          );
          if (!success) {
            allSaved = false;
  ...
  function renderIndicatorTabs() {
    ...
    window.dagenIndicators.indicators.forEach((indicator, index) => {
      ...
      if (tabContentsContainer)
        tabContentsContainer.appendChild(contentContainer);
      ...
      tab.addEventListener("click", () => {
        ...
        if (window.dagenIndicators) {
          window.dagenIndicators.renderIndicatorConfig(
            contentContainer,
            indicator
          );
        }
  ...
  function createNewIndicator() {
    ...
    const newlyAddedTab = modal.querySelector(
      `.tab[data-target="${newIndicator.id}"]`
    );
    if (newlyAddedTab) {
      newlyAddedTab.click();
    }
