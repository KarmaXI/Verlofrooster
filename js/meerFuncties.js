...
async function fetchUserInfo(email) {
  // Check cache
  if (
    USER_INFO_CONFIG.userCache[email] &&
...
async function showUserTooltip(email, element, event) {
  ...
  const tooltip =
    document.getElementById("user-info-tooltip") || createUserTooltip();
    ...
    tooltip.innerHTML = `
            <div class="tooltip-header">
                <div class="tooltip-user-profile">
                        ...
                        ${
                          userInfo.pictureUrl
                            ? `<img src="${userInfo.pictureUrl}" alt="${
                             ...
                             onerror="this.style.display='none'; this.parentNode.textContent='${generateInitials(
                               userInfo.name
                             )}';">`
                    ...
                    <div class="tooltip-user-info">
                        ...
                        ${
                          userInfo.jobTitle
                            ? `<div class="tooltip-job-title">${userInfo.jobTitle}</div>`
                            : ""
                        }
                        ${
                          userInfo.department
                            ? `<div class="tooltip-department">${userInfo.department}</div>`
                            : ""
                        }
                    </div>
                </div>
            ...
            <div class="tooltip-content">
                ...
                ${
                  userInfo.aboutMe &&
                  userInfo.aboutMe !==
                    "Geen persoonlijke informatie beschikbaar"
                    ? `
...
function initializeUserTooltips() {
  ...
  document
    ...
    .addEventListener("mouseout", function (event) {
      ...
      if (
        fromElement.closest(".employee-info") &&
        (!toElement ||
          !fromElement.closest(".employee-info").contains(toElement)) &&
  ...
  document.addEventListener("click", function (event) {
    if (
      !event.target.closest("#user-info-tooltip") &&
...
function applyUserSettings(settings) {
  ...
  if (
    settings.defaultTeam !== "my-team" &&
    settings.defaultTeam !== state.filteredTeam
  ) {
...
async function getUrenPerWeekForEmployee(employeeId) {
  try {
    ...
    if (!response.ok) {
      ...
      console.error(
        `Error fetching UrenPerWeek: ${response.status}`,
        errorText
      );
      return null;
    }
...
async function createUrenPerWeekItem(employeeId, workHoursData) {
  try {
    ...
    const mondayTotal = monday.isFreeDay
      ? 0
      : calculateTotalHours(monday.startTime, monday.endTime);
    const tuesdayTotal = tuesday.isFreeDay
      ? 0
      : calculateTotalHours(tuesday.startTime, tuesday.endTime);
    const wednesdayTotal = wednesday.isFreeDay
      ? 0
      : calculateTotalHours(wednesday.startTime, wednesday.endTime);
    const thursdayTotal = thursday.isFreeDay
      ? 0
      : calculateTotalHours(thursday.startTime, thursday.endTime);
    const fridayTotal = friday.isFreeDay
      ? 0
      : calculateTotalHours(friday.startTime, friday.endTime);
    ...
    const itemData = {
      ...
      // Monday data
      [SP_CONFIG.lists.urenPerWeek.fields.maandagStart]: monday.isFreeDay
        ? ""
        : monday.startTime,
      [SP_CONFIG.lists.urenPerWeek.fields.maandagEind]: monday.isFreeDay
        ? ""
        : monday.endTime,
      [SP_CONFIG.lists.urenPerWeek.fields.maandagTotaal]:
        mondayTotal.toString(),
      [SP_CONFIG.lists.urenPerWeek.fields.maandagSoort]: calculateDayType(
        monday.startTime,
        monday.endTime,
        mondayTotal
      ),
      ...
      // Tuesday data
      [SP_CONFIG.lists.urenPerWeek.fields.dinsdagStart]: tuesday.isFreeDay
        ? ""
        : tuesday.startTime,
      [SP_CONFIG.lists.urenPerWeek.fields.dinsdagEind]: tuesday.isFreeDay
        ? ""
        : tuesday.endTime,
      [SP_CONFIG.lists.urenPerWeek.fields.dinsdagTotaal]:
        tuesdayTotal.toString(),
      [SP_CONFIG.lists.urenPerWeek.fields.dinsdagSoort]: calculateDayType(
        tuesday.startTime,
        tuesday.endTime,
        tuesdayTotal
      ),
      ...
      // Wednesday data
      [SP_CONFIG.lists.urenPerWeek.fields.woensdagStart]: wednesday.isFreeDay
        ? ""
        : wednesday.startTime,
      [SP_CONFIG.lists.urenPerWeek.fields.woensdagEind]: wednesday.isFreeDay
        ? ""
        : wednesday.endTime,
      [SP_CONFIG.lists.urenPerWeek.fields.woensdagTotaal]:
        wednesdayTotal.toString(),
      [SP_CONFIG.lists.urenPerWeek.fields.woensdagSoort]: calculateDayType(
        wednesday.startTime,
        wednesday.endTime,
        wednesdayTotal
      ),
      ...
      // Thursday data
      [SP_CONFIG.lists.urenPerWeek.fields.donderdagStart]: thursday.isFreeDay
        ? ""
        : thursday.startTime,
      [SP_CONFIG.lists.urenPerWeek.fields.donderdagEind]: thursday.isFreeDay
        ? ""
        : thursday.endTime,
      [SP_CONFIG.lists.urenPerWeek.fields.donderdagTotaal]:
        thursdayTotal.toString(),
      [SP_CONFIG.lists.urenPerWeek.fields.donderdagSoort]: calculateDayType(
        thursday.startTime,
        thursday.endTime,
        thursdayTotal
      ),
      ...
      // Friday data
      [SP_CONFIG.lists.urenPerWeek.fields.vrijdagStart]: friday.isFreeDay
        ? ""
        : friday.startTime,
      [SP_CONFIG.lists.urenPerWeek.fields.vrijdagEind]: friday.isFreeDay
        ? ""
        : friday.endTime,
      [SP_CONFIG.lists.urenPerWeek.fields.vrijdagTotaal]:
        fridayTotal.toString(),
      [SP_CONFIG.lists.urenPerWeek.fields.vrijdagSoort]: calculateDayType(
        friday.startTime,
        friday.endTime,
        fridayTotal
      ),
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error(
        "Error creating UrenPerWeek item:",
        response.status,
        errorText
      );
...
async function updateUrenPerWeekItem(itemId, employeeId, workHoursData) {
  try {
    ...
    console.log(
      `Updating UrenPerWeek item ID: ${itemId} for employee ID: ${employeeId}`
    );
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error(
        "Error updating UrenPerWeek item:",
        response.status,
        errorText
      );
...
async function saveEmployeeWorkHours(employeeId, workHoursData) {
  try {
    ...
    if (urenPerWeekListItem) {
      ...
      console.log(
        `Updating existing UrenPerWeek entry with ID: ${urenPerWeekListItem.Id}`
      );
      result = await updateUrenPerWeekItem(
        urenPerWeekListItem.Id,
        medewerkerId,
        workHoursData
      );
    } else {
      // Create new entry
      console.log(
        `Creating new UrenPerWeek entry for employee ID: ${medewerkerId}`
      );
...
function handleSaveUserSettings() {
  ...
  days.forEach((day) => {
    ...
    settings.workSchedule.workDays[day].type = calculateDayType(
      startTime,
      endTime,
      totalHours
    );
...
function initWorkScheduleUI() {
  ...
  const workScheduleContainer = document.getElementById(
    "settingsWorkDaysContainer"
  );
...
function populateWorkScheduleUI(employee) {
  ...
  // Set weekly hours
  document.getElementById("weeklyHoursInput").value =
    employee.weeklyHours || 40;

  // Set work schedule type
  document.getElementById("workScheduleSelect").value =
    employee.workSchedule || "fulltime";
  ...
  days.forEach((day) => {
    ...
    if (startTimeInput && dayData.startTime)
