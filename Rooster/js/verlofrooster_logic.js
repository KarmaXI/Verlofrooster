...
function tekenRooster() {
    ...
    gefilterdeMedewerkers.forEach(medewerker => {
        ...
        dagenTonenInPeriode.forEach((dag, index) => {            
            ...
            let cellIsFilled = false;            // Check if alleVerlofItems has data
            ...
            const verlofOpDag = alleVerlofItems.filter(item => {
                // Extra check to make sure MedewerkerID is available
                ...
                // Log all items for this particular medewerker to help with debugging
                if (itemMedewerkerID === genormaliseerdeMedewerkerUsername) {
                    // Fix: Proper date normalization for comparison
                    const startDate = new Date(item.StartDatum);
                    const endDate = new Date(item.EindDatum);
                    
                    // Normalize dates to remove time portion for proper comparison
                    ...
                    // Check for invalid dates (epoch date 1-1-1970 or NaN dates)
                ...
                // Fix: Date comparison logic with validation 
                const startDate = new Date(item.StartDatum);
                const endDate = new Date(item.EindDatum);
                
                // Normalize all dates to remove time component for comparison
                const startDateNoTime = new Date(startDate.getFullYear(), startDate.getMonth(), startDate.getDate());
                const endDateNoTime = new Date(endDate.getFullYear(), endDate.getMonth(), endDate.getDate());
                
                // Filter out invalid dates and entries from 1970 (epoch/default date)
                const isStartDateValid = !isNaN(startDate.getTime()) && startDate.getFullYear() > 1970;
                const isEndDateValid = !isNaN(endDate.getTime()) && endDate.getFullYear() > 1970;
            ...
            if (verlofOpDag.length > 0) {
                ...
                cellIsFilled = true;                // --- Overlay compensatieuren if present (during verlof display) ---
                // Verbeterde functionaliteit die nog uitgevoerd wordt ook als er al een verlof getoond wordt
                const compensatieOpDag = alleCompensatieUrenItems.filter(item => {
                    if (!item || !item.MedewerkerID || !item.StartCompensatieUren || !item.EindeCompensatieUren) {
                        return false;
                    }
                    
                    // Normaliseer MedewerkerID voor vergelijking
                    const itemMedewerkerID = (typeof item.MedewerkerID === 'string') ? 
                        (typeof window.trimLoginNaamPrefixMachtigingen === 'function' ? 
                            window.trimLoginNaamPrefixMachtigingen(item.MedewerkerID) : 
                            item.MedewerkerID) : 
                        '';
                    ...
                    try {
                        // Converteer datum strings naar Date objecten en verwijder tijdcomponent voor dagvergelijking
                ...
                if (compensatieOpDag.length > 0) {
                    // Overlay SVG/icon, do not clear cell
                    const compItem = compensatieOpDag[0];
                ...
                // Verbeterde check voor CompensatieUren die rekening houdt met datum formats en null waarden
                const compensatieOpDag = alleCompensatieUrenItems.filter(item => {
                    if (!item || !item.MedewerkerID || !item.StartCompensatieUren || !item.EindeCompensatieUren) {
                        return false; // Item heeft niet alle benodigde velden
                    }
                    ...
                    try {
                        ...
                        // Valideer datums
                        if (isNaN(startDatumTijd.getTime()) || isNaN(eindDatumTijd.getTime())) {
                            console.warn(`[VerlofroosterLogic] Ongeldige datum in compensatie item`, item);
                            return false;
                        }
                        
                        // Normaliseer datums voor vergelijking (alleen de dag)
                        const startDate = new Date(startDatumTijd.getFullYear(), startDatumTijd.getMonth(), startDatumTijd.getDate());
                        const endDate = new Date(eindDatumTijd.getFullYear(), eindDatumTijd.getMonth(), eindDatumTijd.getDate());
                        
                        // Debug logging voor specifieke gebruiker
                        if (itemMedewerkerID === genormaliseerdeMedewerkerUsername) {
                            console.log(`[VerlofroosterLogic] Compensatie check voor ${medewerker.Naam}, dag: ${dagNormaal.toLocaleDateString()}, item:`,
                                `start: ${startDate.toLocaleDateString()}, eind: ${endDate.toLocaleDateString()},`,
                                `match: ${startDate <= dagNormaal && endDate >= dagNormaal}`);
                        }
                ...
                if (compensatieOpDag.length > 0) {
                    ...
                    celDiv.style.setProperty('position', 'relative', 'important');
                    // Do not clear cell, just overlay icon
                    ...
                    // Mark as filled to avoid overwriting
                    cellIsFilled = true;
                }
            }
