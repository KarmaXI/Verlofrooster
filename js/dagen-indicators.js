...
class DagenIndicators {
  constructor() {
    ...
    this.patterns = [
      ...
      { id: 'diagonal-right', name: 'Diagonaal (Rechts)', spValue: 'Diagonale lijn (rechts)' },
      { id: 'diagonal-left', name: 'Diagonaal (Links)', spValue: 'Diagonale lijn (links)' },
  ...
  async loadFromSharePoint() {
    ...
    if (this.sharePointConfig.useLocalStorageOnly) {
      console.log('SharePoint API overgeslagen vanwege useLocalStorageOnly instelling');
      return;
    }
    ...
    try {
      ...
      if (this.sharePointConfig.mockDelay > 0) {
        await new Promise(resolve => setTimeout(resolve, this.sharePointConfig.mockDelay));
      }
  ...
  mapSharePointItem(spItem) {
    ...
    const patternObj = this.patterns.find(p => p.spValue === patroonSpValue) || this.patterns[0];
    
    return {
      id: spItem.ID || `sp_${Date.now()}${Math.random().toString(36).substr(2, 5)}`, // Fallback ID
      title: title,
      kleur: kleur,
  ...
  setDefaultIndicators() {
    ...
    this.indicators = [
      { id: 'vvm', title: 'VVM', kleur: this.defaultColors[0], effect: 'diagonal-left' },
      { id: 'vvd', title: 'VVD', kleur: this.defaultColors[1], effect: 'diagonal-right' },
      { id: 'vvo', title: 'VVO', kleur: this.defaultColors[2], effect: 'solid' }
    ];
    this.saveIndicators();
  }
  ...
  async saveToSharePoint(indicator) {
    // Als useLocalStorageOnly is ingeschakeld, sla dan de SharePoint call over
    if (this.sharePointConfig.useLocalStorageOnly) {
      console.log('SharePoint opslaan overgeslagen vanwege useLocalStorageOnly instelling');
    ...
    try {
      ...
      const spPatternValue = this.patterns.find(p => p.id === indicator.effect)?.spValue || 'Effen';
  ...
  renderLegend() {
    const legendSection = document.querySelector('.legend-section[data-type="dagen"]') || 
                          document.getElementById('legendDagenIndicator');
                          
    if (!legendSection) return;
    
    // Maak de inhoud leeg
    const sectionContent = legendSection.querySelector('.legend-section-content') || legendSection;
  ...
  applyIndicatorStyle(element, indicatorId) {
    ...
    element.classList.remove(...Array.from(element.classList).filter(c => c.startsWith('pattern-')));
  ...
  renderIndicatorConfig(container, indicator) {
    ...
    this.defaultColors.forEach(color => {
      ...
      preset.className = `color-preset ${indicator.kleur === color ? 'selected' : ''}`;
    ...
    this.patterns.forEach(pattern => {
      ...
      option.className = `pattern-option pattern-${pattern.id} ${indicator.effect === pattern.id ? 'selected' : ''}`;
